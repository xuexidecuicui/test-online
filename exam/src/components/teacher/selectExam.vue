    ...
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="300">
        <template slot-scope="scope">
          ...
          <el-button @click="editState(scope.row.examCode,scope.row.state)" type="warning" size="small">停用</el-button>
          <el-button @click="editState1(scope.row.examCode,scope.row.state)" type="success" size="small">启用</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    editState(examConde,state){
      if (state==0){
        alert("当前试卷已经被停用请勿重复提交");
        return;
      }
      this.$confirm("确定停用该试卷吗！！","停用提示",{
        confirmButtonText:"确定停用该试卷",
        cancelButtonText:"算了",
      }).then(()=>{
        this.$axios({
          url:`/api/exam/state/${examConde}/${state - 1}`,
          method:"get",
        }).then(res=>{
          alert("停用成功")
          this.getExamInfo();
    ...
    editState1(examConde,state){
      if (state==1){
        alert("当前试卷已经启用请勿重复提交");
        return;
      }
      this.$confirm("确定启用该试卷吗！！","启用用提示",{
        confirmButtonText:"确定启用该试卷",
        cancelButtonText:"算了",
      }).then(()=>{
        this.$axios({
          url:`/api/exam/state/${examConde}/${state + 1}`,
