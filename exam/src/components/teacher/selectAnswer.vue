    ...
    <el-table :data="pagination.records" border :row-class-name="tableRowClassName">
      ...
      <el-table-column fixed="right" label="操作" width="100">
        <template slot-scope="scope">
          <el-button @click="deleteByQuestionId(scope.row.questionId,scope.row.type)" type="danger" size="small">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    deleteByQuestionId(id,type){
      this.$confirm("确定删除该记录吗,该操作不可逆！！！","删除提示",{
        confirmButtonText: '确定删除',
        cancelButtonText: '算了,留着',
      }).then(()=> { //确认删除
        this.$axios({
          url: `/api/answers/${id}/${type}`,
          method: 'post',
        }).then(res => {
          this.getAnswerInfo();
        })
